...
estimate_quantiles <- function(data, indices) {
  sample_data <- data[indices]  # Bootstrap 采样
...
true_80th_percentile <- qexp(0.80, rate = lambda)  # 指数分布下的真实80%分位数
...
So, the posterior for non-uni is X \~ Gamma(31,1952.8), and the posterior for uni X\~Gamma(171,15894.2)
